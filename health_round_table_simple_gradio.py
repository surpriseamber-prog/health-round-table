...
def run_round_table(case, goals, constraints, model_choice):
    ...
    all_responses.append(
        ("Dr. Heart", "Analyzing case from cardiology perspective..."))
...
IMPORTANT: Give practical, actionable advice."""
    try:
        dr_heart_response = chat(
            ...
            dr_heart_system,
            f"Analyze: {case}")
        all_responses.append(("Dr. Heart", dr_heart_response))
    except Exception as e:
        all_responses.append(("Dr. Heart", f"Error: {str(e)}"))
...
IMPORTANT: Give clear numbered recommendations."""
    try:
        ...
        return all_responses
    all_responses.append(("System", "Round Table Complete!"))
    return all_responses
...
with gr.Blocks(title="Health Round Table") as demo:
    ...
    clear_btn.click(
        fn=lambda: [
            None, "", "", ""], outputs=[
            output, case_input, goals_input, constraints_input])
    gr.Markdown(
        "Each specialist reads previous analyses before responding - true collaborative debate.")
